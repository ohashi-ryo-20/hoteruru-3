...
public class ReservationService {
	...
	public Integer calculateAmount(LocalDate checkinDate, LocalDate checkoutDate, Integer price) {
		//ChronoUnit.DAYS.between 2つの日付の日数差を計算する
		long numberOfNights = ChronoUnit.DAYS.between(checkinDate, checkoutDate); //ChronoUnitの戻り値がlong型のため、longを使用する。
		int amount = price * (int)numberOfNights; //普通の宿泊日数ならintの範囲内に収まるためキャストする
		return amount;
	}
}
